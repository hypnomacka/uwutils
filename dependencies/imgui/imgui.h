...
namespace gui
{
    ...
    // Styles
    IMGUI_API void          StyleColors(ImGuiStyle* dst = NULL); // classic imgui style
    ...
    IMGUI_API bool          CustomTab(const char* label, bool selected);
    IMGUI_API bool          KeyBind(const char* label, int* k, int* key_style = 0);
    ...
    IMGUI_API bool          InputText_String(const char* label, std::string* str, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
...
#define IM_NEW(_TYPE)                       new(ImNewWrapper(), gui::MemAlloc(sizeof(_TYPE))) _TYPE
...
// Flags for ImDrawList. Those are set automatically by gui:: functions from ImGuiIO settings, and generally not manipulated directly.
