...
namespace render {
	...
	void draw_beam(color clr, int width, int life, int flag, vec3_t start, vec3_t end);
	void draw_beam_circle(color clr, int width, vec3_t pos);
	void draw_dlight(player_t* player, color clr, int radius);

	namespace fonts {
		extern unsigned long watermark_font;
		extern unsigned long esp_font;
		extern unsigned long esp_flags_font;
		extern unsigned long indicators_font;
		extern unsigned long indicators_font2;
		extern unsigned long indicators_font3;
		extern unsigned long indicators_font4;
		extern unsigned long indicators_font5;
	}
}
