...
void c_kit_parser::initialize() {
	...
	const auto sig_address = util::pattern_scan(xorstr("client.dll"), xorstr("E8 ? ? ? ? FF 76 0C 8D 48 04 E8"));
	...
	printf(xorstr("kit parser initialized\n"));
}